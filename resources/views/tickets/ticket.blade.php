    ...
    <div class="mt-5">
        <div class="col-md-8">
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    @if($ticket->hasFile())
                    <a href="{{$ticket->file()}}">دانلود پیوست
                        {{$ticket->hasFile()}}
                    </a>
                    @endif
                </div>
                <div class="card-footer text-muted">
                    {{$ticket->present()->created_at}} توسط {{$ticket->user->name}}
                </div>
            </div>
        </div>
        ...
        <div class="col-md-8 mt-5">
            <div class="card">
                ...
                <div class="card-footer text-muted">
                    {{$reply->present()->created_at}} توسط {{$reply->repliable->name}}
                </div>
            </div>
        </div>
        @endforeach
        @if(!$ticket->isClosed())
        <div class="col-md-8">
            <form class="mt-5" action="{{route("ticket.reply", $ticket)}}" method="post">
                @csrf
